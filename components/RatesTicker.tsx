...
export const RatesTicker: React.FC = () => {
  return (
    <div className="bg-[#080C17] border-b border-slate-800/80 text-xs py-2 overflow-hidden whitespace-nowrap select-none group relative">
      <div className="animate-marquee-slow flex items-center gap-8">
        {INITIAL_CURRENCIES.concat(INITIAL_CURRENCIES).map((curr, idx) => (
          <div key={`${curr.code}-${idx}`} className="inline-flex items-center gap-2 text-slate-300 shrink-0">
            ...
            <span className="text-slate-400">Sell: <strong className="text-emerald-400">{curr.sellRate.toFixed(2)}</strong></span>
